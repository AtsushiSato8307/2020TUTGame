...
public class CriAtomExPlayer : CriDisposable
{
	 ...
	 * </para>
	 * <para header='備考'>
	 * キュー再生時に本関数を呼び出すと、データ側に設定されているパンタイプ設定を
	 ...
	 * </para>
	 * <para header='注意'>
	 * CriAtomEx.PanType.Unknownを指定して実行した場合、エラーが発生します。<br/>
	 * </para>
	 * </remarks>
	 * <seealso cref='CriAtomExPlayer::Start'/>
	 * <seealso cref='CriAtomExPlayer::Update'/>
	 ...
	 * </para>
	 * <para header='注意'>
	 * 3D音源オブジェクトのパラメータの変更、更新は、AtomExプレーヤの関数ではなく、
	 ...
	 * </para>
	 * <para header='注意'>
	 * 3Dリスナーオブジェクトのパラメータの変更、更新は、AtomExプレーヤの関数ではなく、
	 ...
	 * </para>
	 * <para header='備考'>
	 * キュー再生時に本関数を呼び出すと、データ側に設定されているカテゴリ設定を
	 * <b>上書き</b>します（データ側の設定値は無視されます）。<br/>
	 * 本関数で設定したカテゴリ情報は、ACFのレジスト、アンレジストを行うとクリアされます。<br/>
	 * ACFをレジストしていない状態ではデフォルトカテゴリが使用可能です。
	 * </para>
	 * <para header='注意'>
	 * カテゴリ設定は再生開始前に行ってください。再生中の音声に対してカテゴリ設定の更新を行うと、
	 * カテゴリの再生数カウントが異常になる危険があります。<br/>
	 * </para>
	 * </remarks>
	 ...
	 * </para>
	 * <para header='注意'>
	 * 再生中に ::CriAtomExPlayer::Update 関数、 ::CriAtomExPlayer::UpdateAll 関数によって
	 * 更新することはできません。<br/>
	 * <br/>
	 * HCA-MX用にエンコードされた音声データには、エンベロープが適用されません。<br/>
	 * エンベロープを使用したい音声は、ADXやHCA等、他のコーデックでエンコードしてください。<br/>
	 ...
	 * <remarks>
	 * <para header='説明'>
	 * AtomExプレーヤに設定されている再生パラメータ（AISACコントロール値を含む）をリセットし、
	 * 初期状態（未設定状態）に戻します。<br/>
	 * 本関数呼び出し後、 ::CriAtomExPlayer::Start 関数により再生開始すると、初期状態の再生パラメータで再生されます。<br/>
	 * </para>
	 * <para header='備考'>
	 * 本関数呼び出し後、 ::CriAtomExPlayer::Update 関数、 ::CriAtomExPlayer::UpdateAll 関数を呼び出したとしても、
	 ...
	 * それぞれのハンドルのパラメータリセット関数を呼び出してください。
	 * </para>
	 * </remarks>
	 * <example>
	 * <code>
