...
public sealed class CriWareVersionWindow : EditorWindow
{
	...
	private void OnEnable()
	{
		...
		if (Directory.Exists(Path.Combine(pluginsDirectory, "CriWare")) == false) {
			var fileList = Directory.GetFiles(Application.dataPath, "CriWare.cs", SearchOption.AllDirectories);
			if (fileList.Length > 0) {
				try {
					var pathToCriWare = Directory.GetParent(fileList[0]);
					if (pathToCriWare != null && pathToCriWare.Parent != null) {
						pluginsDirectory = pathToCriWare.Parent.FullName;
					} else {
						Debug.LogWarning("[CRIWARE] Failed to find CriWare plugins folder");
					}
				} catch {
					Debug.LogWarning("[CRIWARE] Failed to find CriWare plugins folder");
				}
	...
	private static List<PluginInfo> LoadPluginInfos(string pluginsDirectory, string[][] pluginBinaryFilenames)
	{
		...
		int itemCnt = pluginBinaryFilenames.Length;
		int currentCnt = 0;
		foreach (var item in pluginBinaryFilenames) {
			EditorUtility.DisplayProgressBar("Gathering Information", "Getting library information for " + item[1], currentCnt / (float)itemCnt);
